  ...
  return (
    <AuthScreenBackground>
      <View style={authScreenStyles.form}>
        <Text style={authScreenStyles.title}>Verify your email</Text>
        <Text style={authScreenStyles.message}>We sent a verification link to:</Text>
        <Text style={authScreenStyles.email}>{email || 'your email address'}</Text>
        <Text style={authScreenStyles.hint}>
          After you verify, tap the button below. You will return to Login, then sign in again while your account
          waits for admin approval.
        </Text>
        <Text style={authScreenStyles.hint}>Check spam/junk if you do not see the email within a few minutes.</Text>

        <Pressable style={authScreenStyles.button} onPress={() => void handleCheckVerified()} disabled={checking || resending}>
          {checking ? (
            <ActivityIndicator color="#fff" />
          ) : (
            <Text style={authScreenStyles.buttonText}>I verified my email</Text>
          )}
        </Pressable>
        ...
        >
          <Text style={authScreenStyles.secondaryButtonText}>
            {resending
              ? 'Sending...'
              : cooldownSeconds > 0
        ...
        <Pressable onPress={() => void handleLogout()}>
          <Text style={authScreenStyles.link}>Use a different account</Text>
        </Pressable>
      </View>
    </AuthScreenBackground>
  );
