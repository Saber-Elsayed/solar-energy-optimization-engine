...
export function useDashboardState() {
  ...
  const [feasibleSelectionId, setFeasibleSelectionId] = useState<string | null>(() => getFeasibleSelection());
    ...
    try {
      ...
      if (energyRes?.ok) {
        ...
        const latestAvailableEnergyWh =
          typeof latestSoc === 'number' ? capacityWhForCalc * latestSocNormalized : capacityWhForCalc;
        setBattery(latest);
  ...
  const selectFeasiblePlan = useCallback((combinationId: string) => {
  ...
  const onSelectCity = useCallback(
    (item: CitySuggestion) => {
      ...
      void fetchForecastPoints(activeDevices, item.name).finally(() => setWeatherLoading(false));
    },
    [activeDevices],
  );
